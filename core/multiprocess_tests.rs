...
#[test]
fn subprocess_db_file_reader_pins_checkpoint_in_other_process() {
    let dir = tempfile::tempdir().unwrap();
    ...
    assert_eq!(
        authority.min_active_reader_frame(),
        Some(backfilled_frame),
        "a reader that bypasses the WAL must still publish its snapshot to other processes"
    );
